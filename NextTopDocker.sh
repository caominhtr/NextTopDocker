...
echo "1) Docking"
echo "Requirement: ID_ligand, ID_protein, ID_ref. Recommended in MOL2 format."
...
echo "2) Scoring only"
echo "Requirement ID_posexx, ID_protein. Recommended in MOL2 format."
echo "In case you want to only score your docking poses without performing docking again"
echo ""
echo "NOTE: Output of NextTopDocker is the probability of one pose to be near-native (RMSD<=2)"
echo ""
read -p "Enter choice [1-2]: " choice
read -p "Enter output directory path (e.g., example/result): " outputdir
echo ""
...
case $choice in
	...
	stt=1
	len=$(wc -l ./example/0_temp.csv | awk '{print $1}')
	...
	done < example/0_temp.csv  >> example/1_smina_score.csv
	...
	mv -f example/*_score.csv "${outputdir}"
	mv "$temp_file" "${outputdir}"
	
	merge_file="${outputdir}/3_merged_score.csv"
	output_file="${outputdir}/4_prediction_full.csv"
	...
	for ID in example/*; do
	    [ -d "$ID" ] || continue
	    ID_base=$(basename "$ID")
	    pro_path=$(find "$ID" -maxdepth 1 -type f -name "*protein*" | head -n1)
	    for lig_path in $(find "$ID" -maxdepth 1 -type f -name "*pose*" | sort); do
		echo "${ID_base},${lig_path},${pro_path}" >> "$temp_file"
	    done
	done
	
	#Score using Smina
	echo "Running SMINA scoring only..."
	
	while IFS=, read -r ID ligand protein; do		
		mkdir -p "example/${ID}/smina_score_only"
	done < example/0_temp_score.csv
	...
	while IFS=, read -r ID ligand protein; do		
		...
		./smina -r "$protein" -l "$ligand" \
		      --score_only > "example/${ID}/smina_score_only/${ligand_base}_smina.txt"    2>/dev/null
		
	done < example/0_temp_score.csv
	
	#Score using GNINA 1.3
	echo "Running GNINA 1.3 rescoring..."
	
	while IFS=, read -r ID ligand protein; do		
		mkdir -p "example/${ID}/gnina_score_only"
	done < example/0_temp_score.csv
	...
	while IFS=, read -r ID ligand protein; do		
		...
		gnina -r "$protein" -l "$ligand" \
		      --score_only > "example/${ID}/gnina_score_only/${ligand_base}_gnina.txt"    2>/dev/null
		
	done < example/0_temp_score.csv
	
	#Extracting score
	merge_file="${outputdir}/1_merged_score.csv"
	while IFS=, read -r ID ligand protein; do		
		ligand_base=$(basename "$ligand")
		ligand_base="${ligand_base%.*}"
		
		smina_score=$(grep '^Affinity' "./example/${ID}/smina_score_only/${ligand_base}_smina.txt" | cut -d ':' -f2 | cut -d '(' -f1 | xargs)
		gnina_score=$(grep '^CNNscore' "./example/${ID}/gnina_score_only/${ligand_base}_gnina.txt" | cut -d ':' -f2 | xargs)
		
		echo "$ID,$ligand_base,$smina_score,$gnina_score" >> "$merge_file"
	done < example/0_temp_score.csv
	
	mv example/0_temp_score.csv "${outputdir}"
	
	output_file="${outputdir}/2_prediction_full.csv"
